...
try {
    ...
    $patients = $database->prepare("SELECT * FROM patient LEFT JOIN departments ON patient.depart_id = departments.id");
    $patients->execute();
        ...
        <div class="content width100 relative">
            <div class="header flex-sb-c p-10 m-b-20">
                ...
                if (isset($_POST['medicine_name'])) {
                    ...
                    $med = $database->prepare("SELECT * FROM patient LEFT JOIN departments ON patient.depart_id = departments.id WHERE p_name LIKE :name");
                ...
                ?>
                    <form class="flex" action="" method="post">
                        <input class="width100" type="search" name="medicine_name" id="" placeholder="البحث عن اسم المريض">
            ...
            <div class="patients_list p-rl-20">
                <div class="patient-header flex-sb-c m-b-15">
                    <h2>قائمة المرضى</h2>
                    <img style="width: 120px;" src="images/download__15_-removebg-preview.png" alt="">
                </div>
                <div class="pt_page width100">
    ...
    if ($med->rowCount() >= 1) {
        foreach($med as $med){
            echo '<tr>
                    ...
                    <td>' . $med['p_date'] . '</td>
                    <td>
                        <a class="width100" href="medical-history.php?name=' . $med['p_name'] . '"><i class="fa-solid fa-ellipsis width100"></i></a>
                    </td>
                </tr>';
        }
    }else{
    echo '<tr>
            <td colspan=5 style="font-size: 20px;font-weight:600">لا يوجد مريض بهذا الاسم</td>
        </tr>';
    }
}else{
                            ...
                            foreach ($patients as $patient) {
                                echo '<tr>
                                        ...
                                        <td>' . $patient['p_date'] . '</td>
                                        <td>
                                            <a class="width100" href="medical-history.php?name=' . $patient['p_name'] . '"><i class="fa-solid fa-ellipsis width100"></i></a>
                                        </td>
                                    </tr>';
                            }
                        }
