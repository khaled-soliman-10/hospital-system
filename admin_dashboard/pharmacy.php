...
try {
    ...
    // echo date("Y-m-d");
            ...
            //doctors
            $doctors = $database->prepare("SELECT * FROM doctors LEFT JOIN doctor_logins ON doctor_logins.doctor_id = doctors.doc_id WHERE role = 'صيدلية'");
            $doctors->execute();

            //day total
            $day = $database->prepare("SELECT SUM(total) AS total FROM ph_receipt  WHERE date = CURRENT_DATE");
            $day->execute();
            ...
            //month total
            $month = $database->prepare("SELECT SUM(total) AS total FROM ph_receipt  WHERE date >= CURRENT_DATE - INTERVAL 1 MONTH");
            $month->execute();
...
<body>
    <div class="page flex">
        ...
        <div class="content main_rpl width100 relative">
            ...
            <div class="rlp flex-c-c p-rl-20 m-b-20">
