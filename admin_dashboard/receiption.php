...
try {
    ...
    //reception`s doctors
    $doctors = $database->prepare("SELECT * FROM doctors LEFT JOIN doctor_logins ON doctor_logins.doctor_id = doctors.doc_id WHERE role = 'استقبال'");
    $doctors->execute();

    //num of patient (offline)
    $off = $database->prepare("SELECT * FROM patient WHERE booking_method = 'offline' AND p_date = CURRENT_DATE");
    $off->execute();
    $numOFF = $off->rowCount();
    
    //num of patient (online)
    $on = $database->prepare("SELECT * FROM patient WHERE booking_method = 'online'");
    $on->execute();
    $numON = $on->rowCount();

    //day total
    $day = $database->prepare("SELECT SUM(price) AS total FROM patient LEFT JOIN departments ON patient.depart_id = departments.id WHERE p_date = CURRENT_DATE AND booking_method = 'offline'");
    $day->execute();
    ...
    //month total
    $month = $database->prepare("SELECT SUM(price) AS total FROM patient LEFT JOIN departments ON patient.depart_id = departments.id WHERE p_date >= CURRENT_DATE - INTERVAL 1 MONTH AND booking_method = 'offline'");
    $month->execute();
...
<body>
    <div class="page flex">
        ...
        <div class="content main_rpl width100 relative">
            <div class="header flex-sb-c p-10 m-b-20">
                ...
                <!-- <div class="search relative">
                    <form class="flex" action="" method="post">
                        <input class="width100" type="search" name="medicine_name" id="" placeholder="البحث عن اسم الموظف">
            ...
            <div class="rlp flex-sb-c p-rl-20 m-b-20" style="justify-content: space-evenly;">
                <div class="statistics first center p-10">
                    ...
                    <button type="submit"><i class="fa-solid fa-file-import"></i> دخل الشهر</button>
                </div>
                <div class="statistics center p-10">
                    <h2 class="m-b-40 m-t-20"> <i class="fa-solid fa-file-invoice-dollar m-l-10"></i> عدد المرضي من الإستقبال</h2>
                    <h2> <span><?php echo $numOFF; ?></span> مريض </h2>
                </div>
                <!-- <div class="statistics center p-10">
                    <h2 class="m-b-40 m-t-20"> <i class="fa-solid fa-wifi m-l-10"></i>عدد المرضي من الأونلاين</h2>
                    <h2> <span><?php //echo $numON; ?></span> مريض </h2>
                </div> -->
            </div>
            <div class="rpl_content p-rl-20 m-r-20 m-l-20">
                <div class="rpl_header flex-sb-c m-b-20">
                    <h2>قائمة موظفين الإستقبال</h2>
