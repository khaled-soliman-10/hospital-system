...
if (isset($_GET['newOne'])) {
    if (isset($_POST['enter'])) {
        ...
        $checkInPhar = $database->prepare("SELECT * FROM store WHERE name = :name");
        ...
        if ($checkInPhar->rowCount() == 1) {
            ...
            if ($quan >= $quantity) {
                ...
                $updatePh = $database->prepare("UPDATE store SET quantity = :new WHERE id = :id");
                ...
                <div class="reciet " style="height: 100%;">
                    <div class="table_reciet width100 m-b-15" style="height: 70%;">
                        <table class="width100">
                            ...
                            if (isset($_GET['newOne']) && isset($_POST['enter'])) {
                                ...
                                //total
                                // $tot = $database->prepare("SELECT SUM(total) AS tot FROM ph_receipt WHERE patient_name = :name AND date = CURRENT_DATE");
                                // $tot->bindParam("name",$_POST['nameOfPatient']);
                                // $tot->execute();
                                // $totC = $tot->fetchObject();
                                // $totResult = $totC->tot;

                                // //date
                                // $date = $database->prepare("SELECT * FROM ph_receipt WHERE patient_name = :name AND date = CURRENT_DATE");
                                // $date->bindParam("name",$_POST['nameOfPatient']);
                                // $date->execute();
                                // if ($date->rowCount()>=1) {
                    ...
                    <div class="total flex-sb-0 m-b-20 p-10">
                        <div class="totalmed">
                            <p>عدد الأدوية : <span><?php if (isset($_GET['newOne']) && isset($_POST['enter'])) //{ echo $rec->rowCount(); }?></span></p>
                        </div>
                        <div class="totalprice">
                            <p>الإجمالي : <span><?php if (isset($_GET['newOne']) && isset($_POST['enter'])) //{ echo $totResult; }?></span></p>
                        </div>
                    </div>
                ...
                <div class="hidden-reciet">
                    <div class="logo">
                        ...
                        <div class="date flex-sb-c m-b-15" style="flex-direction:column-reverse;">
                            <p>الوقت والتاريخ: <span><?php if (isset($_GET['newOne']) && isset($_POST['enter']) && $date->rowCount()>=1) //{ echo $dateResult ." ". $time; }?></span></p>
                        </div>
                    </div>
