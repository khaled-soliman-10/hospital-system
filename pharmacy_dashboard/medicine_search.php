...
try {
    ...
    if (isset($_GET['medicine_name'])) {
        ...
        $med = $database->prepare("SELECT * FROM pharmacy INNER JOIN store ON pharmacy.store_id = store.id WHERE name LIKE :name");
        $med->bindParam("name",$search);
        $med->execute();

        // if ($med->rowCount() == 1) {
        //     $result = $med->fetchObject();
        //     $medName = $result->name;
        //     $medPrice = $result->price;
        //     $medQuan = $result->ph_quantity;
        //     $medPrimary = $result->p_date;
        //     $medExpiry = $result->exp_date;
...
<body>
    <div class="page flex">
        ...
        <div class="content width100 relative">
            ...
            <div class="patient_search m-20 p-20">
                <h2 class="">عرض الدواء من الصيدلية</h2>
                <form action="" class="m-b-20 m-t-20" method="post">
                    <label for="s_p">البحث عن الدواء</label>
                    <input type="search" name="med-name" id="s_p" placeholder="إسم الدواء">
                    <button type="submit" class="b-s" name="inside">بحث</button>
                </form>
                <div class="patient_table">
                    <table>
                        ...
                        if (isset($_POST['inside'])) {
                            ...
                            $inside = $database->prepare("SELECT * FROM pharmacy INNER JOIN store ON pharmacy.store_id = store.id WHERE name LIKE :name");
                            $inside->bindParam("name",$name);
                            $inside->execute();

                            if ($inside->rowCount() >= 1) {
                                foreach($inside as $ins){
                                    echo '<tr>
                                            <td>' . $ins['name'] . '</td>
                                            <td>' . $ins['price'] . '</td>
                                            <td>' . $ins['ph_quantity'] . '</td>
                                            <td>' . $ins['p_date'] . '</td>
                                            <td>' . $ins['exp_date'] . '</td>
                                        </tr>';
                                }
